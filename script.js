...
Puzzle.getNeighbors = function(index) {
	...
	if (index >  3)                           { result.top    = index - 4; }
	if (index < 12)                           { result.bottom = index + 4; }
	if ([0, 4,  8, 12].indexOf(index) === -1) { result.left   = index - 1; }
	if ([3, 7, 11, 15].indexOf(index) === -1) { result.right  = index + 1; }
...
Puzzle.prototype.move = function(direction) {
	...
	var nullPiece = this.nullPiece, neighbors = nullPiece.getNeighbors(), toReplace;
